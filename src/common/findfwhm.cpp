...
double FindFWHM::interpolateX(double x1, double y1, double x2, double y2, double yTarget) {
    return x1 + (yTarget - y1) * (x2 - x1) / (y2 - y1);
}

std::optional<double> FindFWHM::findHalfLeft(QLineSeries *line, double xPeak, double yHalf) {
    for (int i = line->count() - 1; i >= 1; --i) {
        ...
        if ((y1 >= yHalf && y2 <= yHalf) || (y1 <= yHalf && y2 >= yHalf)) {
            return interpolateX(line->at(i).x(), y1, line->at(i - 1).x(), y2, yHalf);
        }
    }
    return std::nullopt;
}

std::optional<double> FindFWHM::findHalfRight(QLineSeries *line, double xPeak, double yHalf) {
