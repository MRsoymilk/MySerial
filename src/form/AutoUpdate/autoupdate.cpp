...
AutoUpdate::AutoUpdate(QWidget *parent) : QWidget(parent), ui(new Ui::AutoUpdate) {
    ui->setupUi(this);
    init();
}
...
QJsonObject AutoUpdate::checkUpdate() {
    ...
    ui->labelUpdateHistory->setText(QString("update history: <a href='%1/update.txt'>%1/update.txt</a>").arg(m_url));
